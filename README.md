...
- 查看/修改引脚和中间件配置：打开 `hal10.ioc`
- 编译工程：打开 `MDK-ARM/hal10.uvprojx`

### 调试建议

- 连接 `USART1` 到串口工具，波特率 `115200`
- 上电后可看到系统启动日志和周期性状态信息
- 舵机动作异常时，优先检查 5 路 PWM 接线、供电能力和舵机安装方向
- OLED 无显示时，优先检查 `PB6/PB7` I2C 接线与外部 Flash 中是否已经烧录位图资源

## 当前实现的注意事项

以下结论来自当前源码实现，而不是注释推测：

- `CommandData_t` 虽然预留了 `duration` 字段，但当前 `CommandMap` 中所有命令的持续时间都为 `0`，因此动作不会自动超时停止，而是持续运行到收到新命令或 `CMD_STOP`
- OLED 与动作任务的同步方式是共享全局变量 `CurrentCommand`，而不是共同消费同一条队列消息
- UART 接收使用中断，发送使用阻塞式 HAL 调用；如果后续日志量继续增加，可能需要改成 DMA 或异步发送
- 表情资源本身不在仓库内，而是依赖外部 `W25Q64` 的预置内容

## 适合继续扩展的方向

- 为表情资源增加离线打包和烧录脚本
- 把动作参数抽离成表驱动，减少硬编码角度
- 为 UART 命令增加帧头、校验和参数字段
- 用事件标志组或消息总线替代 `CurrentCommand` 全局轮询
- 增加自定义应用层 OTA，而不只依赖系统 Bootloader
