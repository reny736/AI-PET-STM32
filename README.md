...
基于 `STM32F103C8T6`、`STM32 HAL` 和 `FreeRTOS` 的AI桌宠动作表情等部分，通过串口与ESP32部分连接。系统通过 `USART1` 接收上位机发送的单字节情绪命令，驱动 5 路舵机执行动作，同时从外部 `W25Q64` SPI Flash 读取表情位图并显示在 `128x64 OLED` 上。
